...
export class PixelCanvas {
  ...
  clear() {
    ...
    const logicalWidth = logical.canvas.width;
    const logicalHeight = logical.canvas.height;
    if (logicalWidth > 0 && logicalHeight > 0) {
      const logicalImage = logical.createImageData(logicalWidth, logicalHeight);
      logical.putImageData(logicalImage, 0, 0);
    }
  }
